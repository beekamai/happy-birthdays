...
<p align="center">
  <img src="docs/media/social-preview.png" alt="Happy Birthdays — cozy self-hostable birthday pages" width="680" />
</p>
...
![Dynamic Open Graph preview](docs/media/og-example.png)
...
```bash
git clone https://github.com/beekamai/happy-birthdays.git
cd happy-birthdays
bun install
...
origin stays bound to `127.0.0.1`.

**One-line install** on a fresh Linux/macOS host — clones the repo and runs the
interactive installer (OS detection, toolchain, `.env`, build, systemd/pm2, nginx):

```bash
curl -fsSL https://raw.githubusercontent.com/beekamai/happy-birthdays/main/scripts/bootstrap.sh | bash
```

Already cloned? Run `./scripts/install.sh` directly, and update later with
...
![Динамическое Open Graph превью](docs/media/og-example.png)
...
остаётся привязан к `127.0.0.1`.

**Установка одной строкой** на чистом Linux/macOS хосте — клонирует репозиторий и
запускает интерактивный установщик (детект ОС, тулчейн, `.env`, сборка, systemd/pm2, nginx):

```bash
curl -fsSL https://raw.githubusercontent.com/beekamai/happy-birthdays/main/scripts/bootstrap.sh | bash
```

Уже склонировали? Запустите `./scripts/install.sh`, а позже обновляйте через
`./scripts/install.sh --update`.
...
<a href="https://star-history.com/#beekamai/happy-birthdays&Date">
  <img src="https://api.star-history.com/svg?repos=beekamai/happy-birthdays&type=Date" alt="Star History Chart" width="600" />
</a>
